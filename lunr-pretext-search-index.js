...
},
{
  "id": "th_unitary",
  "level": "2",
  "url": "s_spectral.html#th_unitary",
  "type": "Theorem",
  "number": "1.23.14",
  "title": "Unitary operators.",
  "body": " Unitary operators   Let , let be a finite-dimensional inner product space, and let . The following statements are equivalent.    is unitary.     for all .     for all .      "
},
{
  "id": "th_spectral_unitary",
  "level": "2",
  "url": "s_spectral.html#th_spectral_unitary",
  "type": "Theorem",
  "number": "1.23.15",
  "title": "Spectral theorem for unitary operators: <span class=\"process-math\">\\(F=\\C\\)<\/span>.",
  "body": " Spectral theorem for unitary operators:   Let , let be a finite-dimensional complex inner product space, and let . The following statements are equivalent.    is unitary     is normal and for all eigenvalues of .      "
},
{
  "id": "s_caylet_ham",
  "level": "1",
  ...
  "body": " Cayley-Hamilton theorem   Polynomial theory   Polynomial divisibility   Let be a field. Recall that is the set of all polynomials with coefficients in .   Divisibility  Given polynomials , we say the  divides  if there is a polynomial satisfying . In this case we call a multiple of , a factor of , and the equation a factorization of .    Irreducible polynomial  A nonconstant polynomial is irreducible if it has no nontrivial factorization: more precisely, if for , then is a constant polynomial or is a constant polynomial.    Division algorithm  Given polynomials with , there is a unique pair of polynomials satisfying the following conditions: . We call and the quotient and remainder , respectively, upon dividing by .         Cayley-Hamilton theorem   Invariant subspaces and characteristic polynomial   Let , where is finite dimensional, and suppose be a -invariant subspace of . Let be the characteristic polynomial of , and let be the characteristic polynomial of the restriction . We have : , the characteristic polynomial of divides the characteristic polynomial of for any -invariant subspace .    Let be a basis of , and extend to a basis . Since is -invariant, it follows easily from that , and that furthermore, . A matrix of the form above is called block upper diagonal . An easy induction argument shows that in general the determinant of block upper diagonal matrices is the product of the determinants of the block matrices along the diagonal. As a result we have . Since the characteristic polynomial of is , and the characteristic polynomial of is ( ), we conclude that .     Multiplicity   Let . Suppose is a root of and let be a factorization of satisfying . The integer is called the multiplicity of the root .     Multiplicity of eigenvalues   Let , where is finite dimensional, let be the characteristic polynomial of , and let be an eigenvalue of .   Algebraic multiplicity  The algebraic multiplicity of is its multiplicity as a root of : the algebraic multiplicity of is the positive integer such that for some satisfying .    Geometric multiplicity  The geometric multiplicity of is defined as .         Let , where is finite dimensional, let be the characteristic polynomial of . Let be an eigenvalue of of algebraic multiplicity : , suppose factors as , where . The geometric multiplicity of is bounded above by the algebraic multiplicity: , .     Let . Since is diagonal (with diagonal entries ), for any basis of , we see that the characteristic polynomial of is . (Recall that the degree of a characteristic polynomial is always equal to the dimension of the underlying space.) By , we have . Since , we cannot have , and thus . (This is intuitive enough, but is actually a consequence of the uniqueness claim of .) But if , it follows (again by unique factorization) that .    -cyclic subspace   Let . Given a vector , the -cyclic subspace generated by is defined as .     -cyclic subspace   Let , let be nonzero, and let .    is a -invariant subspace of .    If is a -invariant subspace of containing , then : , is the smallest -invariant subspace of containing .    If , then there is a such that is a basis of .          This is left to the reader.    Assume is a -invariant subspace containing . Since is -invariant, it contains for all . By , we have .    If is finite dimensional, then the tuple must be linearly dependent. It follows that can be written as a linear combination of the previous for some ( is assumed to be nonzero). Let be minimal in this respect. The tuple is then linearly independent by . We claim spans , and hence is a basis. Since by definition, we need only show that . By the previous result it suffices to show that is -invariant (since ). This is easy: given any , we have for some , in which case , since and .        Cayley-Hamilton theorem   Let , where is finite dimensional, and let be the characteristic polynomial of . We have . In other words, is a root of its own characteristic polynomial.    It suffices to show that for all . This is clearly true for . Let be nonzero and let be the -cyclic subspace it generates. By , has a basis of the form for some , in which case , and we have for some scalars . From the nature of , and the representation of above, it follows that the restriction has matrix representation . An easy proof by induction shows that the characteristic polynomial of a matrix like is . Note that since . Lastly, since by , we have for some polynomial , and hence , as desired.      Minimal polynomial   Minimal polynomial   Let , where is finite dimensional. The minimal polynomial of is the unique monic polynomial of minimal degree satisfying .     Minimal polynomial   Let , where is finite dimensional and let be the minimal polymonial of . If satisfies , then : , we have for some . In particular, if is the characteristic polynomial of , then .    Assume for the polynomial . Using the division algorithm ( ), we can write , where . We thus have . Thus . Since by definition is the nonconstant polynomial of minimal degree satisfying , we conclude that is a constant polynomial. Since , it follows that , the zero polynomial. But then , showing that , as desired.     Minimal polynomial and characteristic polynomial   Let , where is finite dimensional, let and be the characteristic and minimal polynomials of , respectively. If splits completely over as , then splits completely as , where for all . In other words, and both split completely over and have the same set of roots.    Since , unique factorization of polynomials implies for some subset , and exponents . We will show that : equivalently, we will show that for all . Fix and eigenvector . Since , we have , and hence . It follows that Since , and since , we must have . But this is true if and only if for some , or equivalently, .     "
},
{
  "id": "d_poly_divis",
  "level": "2",
...
},
{
  ...
  "body": " Invariant subspaces and characteristic polynomial   Let , where is finite dimensional, and suppose be a -invariant subspace of . Let be the characteristic polynomial of , and let be the characteristic polynomial of the restriction . We have : , the characteristic polynomial of divides the characteristic polynomial of for any -invariant subspace .    Let be a basis of , and extend to a basis . Since is -invariant, it follows easily from that , and that furthermore, . A matrix of the form above is called block upper diagonal . An easy induction argument shows that in general the determinant of block upper diagonal matrices is the product of the determinants of the block matrices along the diagonal. As a result we have . Since the characteristic polynomial of is , and the characteristic polynomial of is ( ), we conclude that .   "
},
{
  "id": "d_multiplicity",
  "level": "2",
...
},
{
  "id": "ss_cayley-ham-6",
  "level": "2",
  "url": "s_caylet_ham.html#ss_cayley-ham-6",
  "type": "Proof",
  "number": "1",
  "title": "",
  "body": " Let . Since is diagonal (with diagonal entries ), for any basis of , we see that the characteristic polynomial of is . (Recall that the degree of a characteristic polynomial is always equal to the dimension of the underlying space.) By , we have . Since , we cannot have , and thus . (This is intuitive enough, but is actually a consequence of the uniqueness claim of .) But if , it follows (again by unique factorization) that .  "
},
{
...
},
{
  ...
  "number": "1.24.8",
  "title": "Cayley-Hamilton theorem.",
  "body": " Cayley-Hamilton theorem   Let , where is finite dimensional, and let be the characteristic polynomial of . We have . In other words, is a root of its own characteristic polynomial.    It suffices to show that for all . This is clearly true for . Let be nonzero and let be the -cyclic subspace it generates. By , has a basis of the form for some , in which case , and we have for some scalars . From the nature of , and the representation of above, it follows that the restriction has matrix representation . An easy proof by induction shows that the characteristic polynomial of a matrix like is . Note that since . Lastly, since by , we have for some polynomial , and hence , as desired.   "
},
{
  "id": "d_min_poly",
  "level": "2",
...
},
{
  ...
  "number": "1.24.10",
  "title": "Minimal polynomial.",
  "body": " Minimal polynomial   Let , where is finite dimensional and let be the minimal polymonial of . If satisfies , then : , we have for some . In particular, if is the characteristic polynomial of , then .    Assume for the polynomial . Using the division algorithm ( ), we can write , where . We thus have . Thus . Since by definition is the nonconstant polynomial of minimal degree satisfying , we conclude that is a constant polynomial. Since , it follows that , the zero polynomial. But then , showing that , as desired.   "
},
{
  "id": "cor_min_poly",
  "level": "2",
...
},
{
  ...
  "body": " Generalized eigenspaces  Let , where is finite dimensional. When is diagonalizable, we can decompose into the -invariant subspaces as a direct sum . This fact aids our understanding of as its restriction to any one of these subspaces is as as simple as we could hope for. In more detail, fixing an eigenvalue and eigenspace , we have , the scaling by operation.  Of course, the downside of this whole discussion is that not all linear operators are diagonalizable! In the next two sections we will address this issue with the theory of generalized eigenspaces and Jordan forms. In this section we will show that although we may not always have a direct sum decomposition of into eigenspaces, we do have a direct sum into generalized eigenspaces (as long as the characteristic polynomial of splits completely over ). Generalized eigenspaces, like their less generalized brothers, are -invariant subspaces; furthermore, the restriction of to one of these generalized eigenspaces, though not as simple as a restriction to an eigenspace, can be thoroughly analyzed with the theory of Jordan forms and bases: the topic of the next section.  Before getting to the heart of the matter generalized eigenspaces, we introduce some more notions and results from the general theory of polynomials over fields that will prove essential in our discussion.   Relatively prime polynomials   Let be a field. Polynomials are relatively prime if they have no nonconstant common factor: , if satisfies and , then for some .  More generally a finite list of polynomials is relatively prime if there is no nonconstant factor common to all the : , if satisfies for all , then for some .     Relatively prime polynomials   Let be a field, and let be a relatively prime list of polynomials. There are polynomials such that .    The proof, though not difficult, lies somewhat outside the scope of this course. This and other results about the theory of polynomials is covered in the ring theory portion of a course in abstract algebra. Moreover, it is an instance of a more general result involving the greatest common divisor of the polynomials .     Factorization of polynomials   Let be a field, and let be a nonzero polynomial.   Irreducible factorization  We can factor in the form , where the factors are pairwise relatively prime, monic irreducible polynomials.    Uniqueness of factorization  The scalar , set of monic irreducible polynomials and powers are all uniquely determined by .       The proof, though not difficult, lies somewhat outside the scope of this course. This and other results about the theory of polynomials is covered in the ring theory portion of a course in abstract algebra.     Generalized eigenspace   Let , where is finite dimensional. Given we define the generalized eigenspace  associated to (or -generalized eigenspace) to be . A generalized -eigenvector of is a nonzero element of .     Generalized eigenspace  Let and fix . The definition of makes clear why we call this a generalized eigenspaces: whereas an eigenvector is a nonzero vector satisfying , a generalized eigenvector is a nonzero vector satisfying for some .    Generalized eigenspaces   Let , where is finite dimensional, and assume the characteristic polynomial of splits completely over the base field .   For all , is a -invariant subspace and .     for .    Let be an eigenvalue of of algebraic multiplicity . We have and .          For each define , which is a subspace by . It is easy to see that and that . Although in general the union of subspaces is not a subspace, the union of nested subspaces is a subspace, as we have had occasion to prove elsewhere. Thus is a subspace. In general is -invariant for any linear transformation . Thus, letting , we see that is -invariant for all . It follows easily that the nested union is also -invariant. Lastly, since a subspace being -invariant implies it is -invariant for any polynomial , and since , where , we see that is -invariant.  Lastly, since , we see that .    Fix scalars , and suppose . By definition, this means that for some positive integers and . Since , the polynomials and are relatively prime. By , we have for some polynomials , and thus also . It follows that . We conclude that .    Since , and since is an eigenvalue, we have . Since is an eigenvalue of algebraic multiplicity , and since the characteristic polynomial splits completely, we have a factorization of the form , where and the are all distinct. Let . Since is -invariant, the characteristic polynomial of divides ( ). Since , cannot be an eigenvalue of for all , and thus for all . It follows that . Lastly, since , it follows from the uniqueness claim in , that , as desired.        Generalized eigenspace decomposition   Let , where is finite dimensional, and assume the characteristic polynomial of factors as , where are the distinct eigenvalues of . We have the following direct sum decomposition of : . It follows that for all . In other words, the dimension of the generalized -eigenspace is equal to the algebraic multiplicity of the eigenvalue .    We will first show that . For each , define . The list is easily seen to be relatively prime: any nonconstant monic common irreducible factor of all the is an irreducible factor of (since ), and thus must be for some ; however, by design ; thus there is no nonconstant irreducible factor common to all the . By , it follows that we have for some polynomials , and hence (evaluating the left- and right-side polynomials above at ) . It follows that for all , we have . Let . We have . Thus . Since is -invariant, it follows that . Setting , we conclude that we can write with for all . This proves .  Finally we show that this subspace sum is in fact a direct sum. Letting , we have , where the last line follows from the fact that . Since the beginning and end of our chain of equalities above are in equal, we conclude that all the inequalities involved are in fact equalities, and hence that . The first equality implies by ; the second equality implies that for all .    "
},
{
...
{
  "id": "s_gen_eigenspace-9",
  "level": "2",
  "url": "s_gen_eigenspace.html#s_gen_eigenspace-9",
  "type": "Remark",
  "number": "1.25.5",
  "title": "Generalized eigenspace.",
  "body": " Generalized eigenspace  Let and fix . The definition of makes clear why we call this a generalized eigenspaces: whereas an eigenvector is a nonzero vector satisfying , a generalized eigenvector is a nonzero vector satisfying for some .  "
},
{
  ...
  "number": "1.25.6",
  "title": "Generalized eigenspaces.",
  "body": " Generalized eigenspaces   Let , where is finite dimensional, and assume the characteristic polynomial of splits completely over the base field .   For all , is a -invariant subspace and .     for .    Let be an eigenvalue of of algebraic multiplicity . We have and .          For each define , which is a subspace by . It is easy to see that and that . Although in general the union of subspaces is not a subspace, the union of nested subspaces is a subspace, as we have had occasion to prove elsewhere. Thus is a subspace. In general is -invariant for any linear transformation . Thus, letting , we see that is -invariant for all . It follows easily that the nested union is also -invariant. Lastly, since a subspace being -invariant implies it is -invariant for any polynomial , and since , where , we see that is -invariant.  Lastly, since , we see that .    Fix scalars , and suppose . By definition, this means that for some positive integers and . Since , the polynomials and are relatively prime. By , we have for some polynomials , and thus also . It follows that . We conclude that .    Since , and since is an eigenvalue, we have . Since is an eigenvalue of algebraic multiplicity , and since the characteristic polynomial splits completely, we have a factorization of the form , where and the are all distinct. Let . Since is -invariant, the characteristic polynomial of divides ( ). Since , cannot be an eigenvalue of for all , and thus for all . It follows that . Lastly, since , it follows from the uniqueness claim in , that , as desired.      "
},
{
  ...
  "body": " Generalized eigenspace decomposition   Let , where is finite dimensional, and assume the characteristic polynomial of factors as , where are the distinct eigenvalues of . We have the following direct sum decomposition of : . It follows that for all . In other words, the dimension of the generalized -eigenspace is equal to the algebraic multiplicity of the eigenvalue .    We will first show that . For each , define . The list is easily seen to be relatively prime: any nonconstant monic common irreducible factor of all the is an irreducible factor of (since ), and thus must be for some ; however, by design ; thus there is no nonconstant irreducible factor common to all the . By , it follows that we have for some polynomials , and hence (evaluating the left- and right-side polynomials above at ) . It follows that for all , we have . Let . We have . Thus . Since is -invariant, it follows that . Setting , we conclude that we can write with for all . This proves .  Finally we show that this subspace sum is in fact a direct sum. Letting , we have , where the last line follows from the fact that . Since the beginning and end of our chain of equalities above are in equal, we conclude that all the inequalities involved are in fact equalities, and hence that . The first equality implies by ; the second equality implies that for all .   "
},
{
  "id": "s_jordan_form",
  "level": "1",
  ...
  "number": "1.26",
  "title": "Jordan form",
  "body": " Jordan form   Jordan block   Let be a field. Given and positive integer , the matrix is called a -Jordan block .     Jordan basis   Let , where is finite dimensional. A Jordan basis of is a basis for which is block diagonal of the form where each is a -Jordan block for some . The matrix is called a Jordan form of in this case.     Cycle of generalized eigenvectors   Let where is finite dimensional, and let be an eigenvalue of . A cycle of generalized eigenvectors of corresponding to is a tuple of the form where and is the smallest positive integer satisfying . The vector is called the initial vector of the cycle; the vector is called the end vector of the cycle.     Jordan blocks   Let where is finite dimensional.   is a Jordan block if and only if is a cycle of generalized eigenvectors.   is a Jordan form if and only if is a concatenation of disjoint cycles of generalized eigenvectors.  A cycle of generalized eigenvectors is linearly independent, and its initial vector is an eigenvector.  If are cycles of generalized eigenvectors corresponding to a fixed whose initial vectors are linearly independent, then the concatenation is linearly independent.         Suppose is a cycle of generalized eigenvectors of that is a basis of . Let , and let . (In other words, is the -th element of .) Clearly we have for all . Since is a cycle, we also have . Thus, using , we have . Since the matrix representation operator is linear, we have . The reverse implication (that if is a Jordan block, then is a cycle of generalized eigenvectors), follows essentially from the exact same observations in reverse.    This follows easily from (1) and the fact that is -invariant for any cycle of generalized eigenvectors .    This follows from , where we showed that if is the smallest positive integer such that , then is linearly independent.    We prove the result by induction on the sum of the lengths , the base case being trivial. Assume now that the result is true for any collection cycles of generalized -eigenvectors whose lengths sum to , and suppose that we have cycles whose initial vectors are linearly independent, and whose lengths satisfy . Let , and for each let . By definition, this means that . If for all , then clearly the concatenation is linearly independent. Otherwise, observe that the collection of shortened tuples obtained by deleting ( popping ) the last element of each of length greater than 1 are cycles with the same linearly independent initial vectors. Since the sum of the lengths of these cycles is less than , the induction hypothesis applies to the : , the concatenation of their elements is linearly independent.  Now suppose we have a linear combination of the elements of equal to : . Now apply the linear operator to both sides of this equation: . Note that this is a linear combination of the elements of the shortened cycles . Since their concatenation is linearly independent, we conclude that all of the coefficients with , are equal to 0. Thus the original linear combination is of the form . By assumption the tuple of initial vectors is linearly independent. We conclude that for all , and thus that the tuple is linearly independent, as desired.        Jordan form   Let where is finite dimensional, and suppose all roots of the characteristic polynomial of lie in .  There is a Jordan basis for .  The corresponding Jordan form is unique up to a permutation of the Jordan blocks .      Proof of (1)  We first prove that the existence of a Jordan basis. Let be the decomposition of into generalized eigenspaces, as per . Observe that since each is -invariant, if we can find Jordan bases for , then the concatenation is a Jordan basis for . Thus it suffices to prove the result for linear operators whose characteristic polynomial splits as .  We now prove by induction on , that if is a linear operator with characteristic polynomial , , then has a Jordan basis. The base case is trivial, since any operator on a -dimensional space has a matrix representation of the form , which is clearly a Jordan form.  Assume now the result applies for any operator on any -dimensional vector space, . Given with and with characteristic polynomial , let , and let . Since is an eigenvalue of , is non-invertible, and thus . It follows that . Moreover, is - and thus -invariant, and since the characteristic polynomial of divides , we see that has characteristic polynomial . Thus we may apply the induction hypothesis to conclude that has a Jordan basis that is a concatenation of the cycles of generalized -eigenvectors . Write each as , and observe that by definition we have for all . Since the end vector of each cycle is an element of , for all we can find a vector satisfying , yielding lengthened cycles of generalized -eigenvectors for all .  Lastly, since the concatenation of the original cycles is linearly independent, in particular, the tuple of their initial vectors is linearly independent. Since furthermore these elements are elements of ( ), we can extend this tuple to a basis of . We claim that the concatenation of the cycles is our desired Jordan basis of . Since the initial vectors of these cycles is precisely the linearly independent tuple in , it follows from that is linearly independent. To prove is a basis it suffices to show that . Let , the starting Jordan basis of . We created by adding one extra vector for each of the cycles , as well as the vectors added to get a basis of the -dimensional subspace . Thus we have , as desired.    Proof of (2)  The uniqueness of the Jordan form follows from the fact that for each generalized eigenspace the number of Jordan blocks for as well as the list of lengths of these sequences can be expressed solely in terms of for various , and hence depend solely on itself and not on the particular manner of choosing a Jordan basis. Let's see why this is so.  Assume has characteristic polynomial , let , and suppose is a Jordan basis of . Without loss of generality, we may assume that for all . Since the are cycles of generalized -eigenvectors, we have , where for all . The cycle structure allows us to easily choose subtuples of the Jordan basis that serve as bases of for various . Consider first. We claim that the tuple consisting of initial vectors of the is a basis of . Indeed, given any , since is a basis of , we can write for some , whence , where the last line follows from linear independence. Thus if and only if , showing that the tuple in spans . Since that tuple is linearly independent, we conclude it is a basis, as claimed.  A similar argument, replacing with in our computations above, shows that the tuple is a basis of . The idea behind these arguments is nicely visualized by arranging the cycles in vertical columns as follows: . Applying to the basis moves vectors in the top row to and moves all other vectors up one row. Moreover, from our discussion above, we see that the top rows of the array form a basis for , and thus that the number of vectors in the -th row of the array is given by , for all . This means the precise shape of the array is determined by the values , which depend only on . Knowing the precise shape of this array tells us in turn the essential details of the Jordan block decomposition. For example, we have . Similarly, we can deduce the length of each column in our array, and hence (equivalently, the size of the -th Jordan block), from the sequence . Indeed, we have , as one easily verifies. For example, if the sequence is , then we deduce that .     Jordan form   Let , where is finite dimensional. Assume that the characteristic polynomials of and both split completely over the base field. We say and have the same Jordan form if they have Jordan forms that differ by a permutation of the Jordan blocks.     Dot diagram   Let , where is finite dimensional. Fix an eigenvalue , and let be a Jordan basis of , where each is a cycle of generalized -eigenvectors satisfying for all . The dot diagram of is an staircase pattern of dots constructed in the manner described below. For all , let be the number of dots in the -th row of the dot diagram.   Dots are placed into a rectangular grid containing rows and columns. For all , the -th row contains dots, and these are placed consecutively starting in the leftmost column.    We have for all .   The dot diagram is interpreted as follows.   For any , the total number of dots in the first rows of the diagram is equal to : , . In particular, we have . Thus the number of -Jordan blocks of is equal to the number of dots in the first row of the diagram, which is equal to .    For each , the number of dots in the -th column of the diagram is equal to the length of . Equivalently, is the size of the -th -Jordan block.       We illustrate how to compute a normal form for a matrix transformation in . Since those examples take up quite a lot of space, we place them at the end of the section. In the meantime, we list some interesting theoretical consequences of .   Minimal polynomial from Jordan form   Let , where is finite dimensional, and suppose the characteristic polynomial of factors as , where for , and . Let be the minimal polynomial of . We have , where is the maximum size of a -Jordan block appearing in a Jordan form for .    Let , and for each , let and let be the minimal polynomial of . Since , we have for all , and hence for all ; since further, we have for some with , it follows that . Lastly, using a decomposition , with , we see easily that , and hence that . Since and are both monic, and are each a factor of the other, it follows that . In other words, the minimal polynomial of is the product of the minimal polynomials of the restrictions .  Since we compute a Jordan form of be computing Jordan forms for each restriction , it now suffices to prove the claim for . Thus without loss of generality, we may assume that has characteristic polynomial . Let be a Jordan basis of , and let be the corresponding Jordan form of , where each is a -Jordan block of dimension . Since each has characteristic polynomial , we see that the minimal polynomial of is of the form for some . An easy proof by induction now shows that . In other words, in general the minimal polynomial of a -Jordan block of dimension is , as is easily shown. Lastly, from the block diagonal description of , it follows that , for any polynomial . We conclude that . It follows that the minimal polynomial of , and hence of is , where is the maximum size of the Jordan blocks , as desired.     Similarity   Let and suppose both characteristic polynomials factor completely over . The following statements are equivalent.    and are similar (i.e. for some invertible ).     and have the same Jordan form.       Assume and are similar, and thus that there is an invertible matrix such that . Let be a Jordan form for . By definition, there is a basis such that . Let be the standard basis, and let . Using the change of basis formula, we have , and thus . Equivalently, letting be the basis consisting of the columns of , we see that . Thus and have the same Jordan form.  Now assume and have the same Jordan form . Reasoning as above, this implies that there are invertible matrices and such that . It follows that , showing that and are similar.     Diagonal plus nilpotent   Let , where is finite-dimensional, and assume the characteristic polynomial of splits completely over the base field. There are operators satisfying the following conditions:    ;     is diagonalizable;     is nilpotent ( , for some ).   In other words is the sum of a diagonalizable and a nilpotent operator.    Let be a Jordan basis of , and let be its corresponding Jordan form. It is not difficult to see that we can write , where is diagonal and is upper triangular with zeros along the diagonal. Moreover, this implies that for some : , any such is nilpotent. Since the matrix representation operator is an isomorphism of to , , there are operators satisfying . Furthermore, since is diagonal and is a nilpotent matrix, it follows that is diagonalizable and is a nilpotent operator. Lastly, since , and since is bijective, it follows that .     Square-roots of complex matrices   Let be an invertible matrix. There is a matrix such that . In other words, every invertible complex matrix has a square root.  It follows that any invertible operator on a finite-dimensional complex vector space ( ) has a square-root.    From calculus we learn that the infinite series converges to for all . It follows from power series theory that for all , the truncated series satisfies for some polynomial . Thus if where is a nilpotent matrix satisfying , then since . This proves that any matrix of the form has a square root , and that in fact we may take for sufficiently large.  Now assume is invertible. From and Jordan theory, we can write , where is diagonal and is nilpotent, and furthermore the diagonal entries of are the eigenvalues of , which are all nonzero. It follows that is diagonal. Write . Since is diagonal, it commutes with all matrices, and it follows that is also nilpotent. By the previous paragraph there is a square-root of . Let be a diagonal square-root of (it is easy to see such a matrix exists), and let . We have .    At last we are ready to do some computations around Jordan forms and bases. In order to illustrate the subtleties of this analysis we need to work with larger matrices than usual, and this in turn compels us to reach for some computing technology. The wonderful open source computer algebra system Sage serves our needs for this task.   Computing Jordan form   Consider the matrix transformation , where . Compute the characteristic polynomial of .   We see that . This tells us that decomposes into two generalized eigenspaces , and moreover (looking at the multiplicities), that .  Now compute the dot diagram for . Since , we know that , and thus we only need to look at for .   Let be the number of dots in the -th row of our dot diagram for . From the computation above, we see that . The dot diagram for is thus and the Jordan form of breaks into 3 blocks of size , , and . .  Now compute the dot diagram for .   The corresponding dot diagram now consists of a single column and thus the Jordan form for consists of a single Jordan block . In summary, we conclude that there is a Jordan basis of the form , where for some nonzero vectors , and we have . Let us confirm with Sage.      Computing Jordan form   Let , where .   Use the SageCells below to compute a Jordan normal form of in a step-by-step fashion, as in . Confirm your answer using the jordan_form() command.       Having computed the Jordan normal form for , hazard a guess as to what happens to as we let . Verify or falsify your guess in the SageCell below.        Having successfully computed a few Jordan forms for linear operators, we should point out that we have not yet computed an actual Jordan basis . An algorithm for computing Jordan bases can be extracted from the proof of , though it requires significantly more computational work. We content ourselves with illustrating the method with a manageable example, done systematically.   Jordan basis   Let , where . Proceeding as in the examples above, we find that the characteristic polynomial of is , and that a normal form of is . A Jordan basis of will thus consist of three cycles . It is illuminating to arrange this basis as in our dot diagrams: Following the argument in , we will first compute a Jordan basis for , then modify this appropriately to get a Jordan basis of , then repeat the previous step to get a Jordan basis of .  From the diagram above, we see that , and that . Thus any basis of will consist of a single eigenvector, and thus will automatically be a Jordan basis. As we see below, we can choose for the sole element of this basis.   Following the induction argument in the proof of , to create a Jordan basis of , we (a) lengthen the cycle to a cycle , where and , (b) extend to a basis of , producing the two cycles of generalized eigenvectors , which form a basis of . From the computation of above, we see that . Thus we can pick . To extend to a basis of , we first use Sage to compute a basis of this space.   Since , we see that we can add either of the basis elements above to to get a basis. We choose . A Jordan basis of is then obtained by concatenating the two cyles of generalized eigenvectors below . We now repeat this process to transform the Jordan basis of into a Jordan basis of . First we lengthen the two cycles above. Clearly the first cycle can be lengthened by appending . For the second we find satisfying . As we show below, we can choose .   This yields the two cycles . Lastly, we extend to a basis of . This can be done systematically by putting in as the first two columns of a matrix, followed by a complete basis for , and then using the column space algorithm to contract to a basis. The computation below creates this augmented matrix; we see by inspection that we can pick .   Putting it all together, we obtain the following cycles, whose concatenation is a Jordan basis for : Time to put our theory to the test. We should obtain the Jordan form of by the change of basis formula , where    We did it!    "
},
{
  "id": "d_jordan_block",
  "level": "2",
...
},
{
  ...
  "number": "1.26.4",
  "title": "Jordan blocks.",
  "body": " Jordan blocks   Let where is finite dimensional.   is a Jordan block if and only if is a cycle of generalized eigenvectors.   is a Jordan form if and only if is a concatenation of disjoint cycles of generalized eigenvectors.  A cycle of generalized eigenvectors is linearly independent, and its initial vector is an eigenvector.  If are cycles of generalized eigenvectors corresponding to a fixed whose initial vectors are linearly independent, then the concatenation is linearly independent.         Suppose is a cycle of generalized eigenvectors of that is a basis of . Let , and let . (In other words, is the -th element of .) Clearly we have for all . Since is a cycle, we also have . Thus, using , we have . Since the matrix representation operator is linear, we have . The reverse implication (that if is a Jordan block, then is a cycle of generalized eigenvectors), follows essentially from the exact same observations in reverse.    This follows easily from (1) and the fact that is -invariant for any cycle of generalized eigenvectors .    This follows from , where we showed that if is the smallest positive integer such that , then is linearly independent.    We prove the result by induction on the sum of the lengths , the base case being trivial. Assume now that the result is true for any collection cycles of generalized -eigenvectors whose lengths sum to , and suppose that we have cycles whose initial vectors are linearly independent, and whose lengths satisfy . Let , and for each let . By definition, this means that . If for all , then clearly the concatenation is linearly independent. Otherwise, observe that the collection of shortened tuples obtained by deleting ( popping ) the last element of each of length greater than 1 are cycles with the same linearly independent initial vectors. Since the sum of the lengths of these cycles is less than , the induction hypothesis applies to the : , the concatenation of their elements is linearly independent.  Now suppose we have a linear combination of the elements of equal to : . Now apply the linear operator to both sides of this equation: . Note that this is a linear combination of the elements of the shortened cycles . Since their concatenation is linearly independent, we conclude that all of the coefficients with , are equal to 0. Thus the original linear combination is of the form . By assumption the tuple of initial vectors is linearly independent. We conclude that for all , and thus that the tuple is linearly independent, as desired.      "
},
{
  "id": "th_jordan_form",
  "level": "2",
  ...
  "number": "1.26.5",
  "title": "Jordan form.",
  "body": " Jordan form   Let where is finite dimensional, and suppose all roots of the characteristic polynomial of lie in .  There is a Jordan basis for .  The corresponding Jordan form is unique up to a permutation of the Jordan blocks .      Proof of (1)  We first prove that the existence of a Jordan basis. Let be the decomposition of into generalized eigenspaces, as per . Observe that since each is -invariant, if we can find Jordan bases for , then the concatenation is a Jordan basis for . Thus it suffices to prove the result for linear operators whose characteristic polynomial splits as .  We now prove by induction on , that if is a linear operator with characteristic polynomial , , then has a Jordan basis. The base case is trivial, since any operator on a -dimensional space has a matrix representation of the form , which is clearly a Jordan form.  Assume now the result applies for any operator on any -dimensional vector space, . Given with and with characteristic polynomial , let , and let . Since is an eigenvalue of , is non-invertible, and thus . It follows that . Moreover, is - and thus -invariant, and since the characteristic polynomial of divides , we see that has characteristic polynomial . Thus we may apply the induction hypothesis to conclude that has a Jordan basis that is a concatenation of the cycles of generalized -eigenvectors . Write each as , and observe that by definition we have for all . Since the end vector of each cycle is an element of , for all we can find a vector satisfying , yielding lengthened cycles of generalized -eigenvectors for all .  Lastly, since the concatenation of the original cycles is linearly independent, in particular, the tuple of their initial vectors is linearly independent. Since furthermore these elements are elements of ( ), we can extend this tuple to a basis of . We claim that the concatenation of the cycles is our desired Jordan basis of . Since the initial vectors of these cycles is precisely the linearly independent tuple in , it follows from that is linearly independent. To prove is a basis it suffices to show that . Let , the starting Jordan basis of . We created by adding one extra vector for each of the cycles , as well as the vectors added to get a basis of the -dimensional subspace . Thus we have , as desired.    Proof of (2)  The uniqueness of the Jordan form follows from the fact that for each generalized eigenspace the number of Jordan blocks for as well as the list of lengths of these sequences can be expressed solely in terms of for various , and hence depend solely on itself and not on the particular manner of choosing a Jordan basis. Let's see why this is so.  Assume has characteristic polynomial , let , and suppose is a Jordan basis of . Without loss of generality, we may assume that for all . Since the are cycles of generalized -eigenvectors, we have , where for all . The cycle structure allows us to easily choose subtuples of the Jordan basis that serve as bases of for various . Consider first. We claim that the tuple consisting of initial vectors of the is a basis of . Indeed, given any , since is a basis of , we can write for some , whence , where the last line follows from linear independence. Thus if and only if , showing that the tuple in spans . Since that tuple is linearly independent, we conclude it is a basis, as claimed.  A similar argument, replacing with in our computations above, shows that the tuple is a basis of . The idea behind these arguments is nicely visualized by arranging the cycles in vertical columns as follows: . Applying to the basis moves vectors in the top row to and moves all other vectors up one row. Moreover, from our discussion above, we see that the top rows of the array form a basis for , and thus that the number of vectors in the -th row of the array is given by , for all . This means the precise shape of the array is determined by the values , which depend only on . Knowing the precise shape of this array tells us in turn the essential details of the Jordan block decomposition. For example, we have . Similarly, we can deduce the length of each column in our array, and hence (equivalently, the size of the -th Jordan block), from the sequence . Indeed, we have , as one easily verifies. For example, if the sequence is , then we deduce that .   "
},
{
  "id": "d_Jordan_form",
  "level": "2",
...
{
  "id": "th_min_poly_Jordan",
  "level": "2",
  "url": "s_jordan_form.html#th_min_poly_Jordan",
  "type": "Theorem",
  "number": "1.26.8",
  "title": "Minimal polynomial from Jordan form.",
  "body": " Minimal polynomial from Jordan form   Let , where is finite dimensional, and suppose the characteristic polynomial of factors as , where for , and . Let be the minimal polynomial of . We have , where is the maximum size of a -Jordan block appearing in a Jordan form for .    Let , and for each , let and let be the minimal polynomial of . Since , we have for all , and hence for all ; since further, we have for some with , it follows that . Lastly, using a decomposition , with , we see easily that , and hence that . Since and are both monic, and are each a factor of the other, it follows that . In other words, the minimal polynomial of is the product of the minimal polynomials of the restrictions .  Since we compute a Jordan form of be computing Jordan forms for each restriction , it now suffices to prove the claim for . Thus without loss of generality, we may assume that has characteristic polynomial . Let be a Jordan basis of , and let be the corresponding Jordan form of , where each is a -Jordan block of dimension . Since each has characteristic polynomial , we see that the minimal polynomial of is of the form for some . An easy proof by induction now shows that . In other words, in general the minimal polynomial of a -Jordan block of dimension is , as is easily shown. Lastly, from the block diagonal description of , it follows that , for any polynomial . We conclude that . It follows that the minimal polynomial of , and hence of is , where is the maximum size of the Jordan blocks , as desired.   "
},
{
  "id": "th_similarity",
  "level": "2",
  ...
  "number": "1.26.9",
  "title": "Similarity.",
  "body": " Similarity   Let and suppose both characteristic polynomials factor completely over . The following statements are equivalent.    and are similar (i.e. for some invertible ).     and have the same Jordan form.       Assume and are similar, and thus that there is an invertible matrix such that . Let be a Jordan form for . By definition, there is a basis such that . Let be the standard basis, and let . Using the change of basis formula, we have , and thus . Equivalently, letting be the basis consisting of the columns of , we see that . Thus and have the same Jordan form.  Now assume and have the same Jordan form . Reasoning as above, this implies that there are invertible matrices and such that . It follows that , showing that and are similar.   "
},
{
  ...
  "body": " Diagonal plus nilpotent   Let , where is finite-dimensional, and assume the characteristic polynomial of splits completely over the base field. There are operators satisfying the following conditions:    ;     is diagonalizable;     is nilpotent ( , for some ).   In other words is the sum of a diagonalizable and a nilpotent operator.    Let be a Jordan basis of , and let be its corresponding Jordan form. It is not difficult to see that we can write , where is diagonal and is upper triangular with zeros along the diagonal. Moreover, this implies that for some : , any such is nilpotent. Since the matrix representation operator is an isomorphism of to , , there are operators satisfying . Furthermore, since is diagonal and is a nilpotent matrix, it follows that is diagonalizable and is a nilpotent operator. Lastly, since , and since is bijective, it follows that .   "
},
{
  "id": "th_square_roots",
  "level": "2",
  ...
  "body": " Square-roots of complex matrices   Let be an invertible matrix. There is a matrix such that . In other words, every invertible complex matrix has a square root.  It follows that any invertible operator on a finite-dimensional complex vector space ( ) has a square-root.    From calculus we learn that the infinite series converges to for all . It follows from power series theory that for all , the truncated series satisfies for some polynomial . Thus if where is a nilpotent matrix satisfying , then since . This proves that any matrix of the form has a square root , and that in fact we may take for sufficiently large.  Now assume is invertible. From and Jordan theory, we can write , where is diagonal and is nilpotent, and furthermore the diagonal entries of are the eigenvalues of , which are all nonzero. It follows that is diagonal. Write . Since is diagonal, it commutes with all matrices, and it follows that is also nilpotent. By the previous paragraph there is a square-root of . Let be a diagonal square-root of (it is easy to see such a matrix exists), and let . We have .   "
},
{
  "id": "eg_jordan_form_1",
  "level": "2",
